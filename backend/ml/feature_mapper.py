...
class PredictionRequest(BaseModel):
    ...
    @validator('soil')
    def normalize_soil_data(cls, v):
        """Normalize soil type during validation"""
        # Create a dict copy to normalize
        soil_dict = v.dict() if hasattr(v, 'dict') else v
...
class FeatureMapper:
    ...
    def extract_features(request: PredictionRequest) -> pd.DataFrame:
        ...
        Args:
            request: Validated Pydantic request object
        ...
        features = {
            ...
            'Phosphorous': request.soil.phosphorus,  # Map phosphorus -> Phosphorous
            'Potassium': request.soil.potassium,
            'Carbon': request.soil.carbon,
            'Soil': request.soil.soilType  # Already normalized
        }
